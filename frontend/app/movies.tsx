...
export default function MoviesScreen() {
  ...
  const [catalog, setCatalog] = useState<{ items: MovieItem[]; source?: string; attribution?: string }>({ items: [] });
  const [library, setLibrary] = useState<MovieLibraryRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [savingKey, setSavingKey] = useState("");

  const columns = width >= 1020 ? 4 : width >= 760 ? 3 : width >= 520 ? 2 : 1;
  ...
  const libraryItems = useMemo(() => {
    return library
      .map((row) => normalizeLibraryRow(row))
  ...
  const items = useMemo(() => {
    ...
    return base.map((item) => {
      ...
      return { ...item, favorite: Boolean(mark?.favorite || item.favorite), watchlist: Boolean(mark?.watchlist || item.watchlist), watched: Boolean(mark?.watched || item.watched) };
    });
  }, [catalog.items, libraryItems, libraryMarks, section]);

  const load = async (nextSection = section, nextQuery = query) => {
    try {
      setLoading(true);
      const params = new URLSearchParams();
      params.set("kind", nextSection === "library" ? "all" : nextSection);
      ...
      setCatalog({ items: catalogPayload.items || [], source: catalogPayload.source, attribution: catalogPayload.attribution });
      setLibrary(libraryPayload.items || []);
    } catch (e: any) {
      setCatalog({ items: MOVIE_LOCAL_FALLBACK, source: "fallback", attribution: TMDB_ATTRIBUTION });
      setLibrary([]);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      load(section, query).catch(() => undefined);
    }, query.trim() ? 350 : 0);
          ...
          <View style={styles.statusBand}>
            <View style={styles.statusLine}>
              ...
              <Text style={styles.statusText}>
                {catalog.source === "tmdb" ? "Catalogue synchronise TMDB" : "Mode secours active: films sauvegardes et listes disponibles"}
              </Text>
            </View>
          ...
          {loading ? (
            ...
            <View style={styles.grid}>
              {items.map((item, index) => (
                <Animated.View key={`${item.media_type}-${item.id || item.tmdb_id}`} entering={FadeInUp.delay(Math.min(index, 16) * 35)} style={{ width: cardWidth as any, padding: 8 }}>
                  <MovieCard item={item} savingKey={savingKey} onToggle={toggle} />
                </Animated.View>
              ))}
            </View>
          ) : (
            <View style={styles.empty}>
              ...
              <GhostButton title="Recharger" onPress={() => load(section, query)} icon={<Ionicons name="refresh" size={16} color={Colors.cyan} />} />
            </View>
          )}
        </ScrollView>
      </SafeAreaView>
...
function MovieCard({ item, onToggle, savingKey }: { item: MovieItem; onToggle: (item: MovieItem, listType: "favorite" | "watchlist" | "watched") => void; savingKey: string }) {
  ...
  return (
    <View style={styles.movieCard}>
      ...
      <View style={styles.movieBody}>
        ...
        <Text style={styles.overview} numberOfLines={3}>{item.overview || "Synopsis indisponible pour le moment."}</Text>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: "row", alignItems: "center", gap: 12, paddingHorizontal: 18, paddingTop: 10, paddingBottom: 8 },
  ...
  freeBadge: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 10, height: 34, borderRadius: 999, backgroundColor: Colors.green },
  ...
  sectionTrack: { paddingHorizontal: 14, paddingTop: 12, paddingBottom: 6, gap: 8 },
  ...
  movieCard: { borderRadius: 18, overflow: "hidden", backgroundColor: "rgba(12,12,20,0.94)", borderWidth: 1, borderColor: Colors.border, minHeight: 438 },
